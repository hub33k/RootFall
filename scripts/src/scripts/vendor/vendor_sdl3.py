...
HASH = "edfbfa27b44d7f8963ddffe536773227329755c1"

app = typer.Typer(help="CLI tool to manage SDL3 vendor tasks.")


# TODO (hub33k): make generic command for vendors (update, patch, etc.)

@app.command()
def update():
    """Update SDL3 sources."""
    typer.echo("Updating SDL3...")


@app.command()
def patch():
    """Patch SDL3 sources."""
    typer.echo("Patching SDL3...")


def main() -> None:
    """
    Entry point for `uv run vendor_sdl3`.
    """
    if len(sys.argv) == 1:
        print("Default: build SDL3 (runs if no subcommand is given).")
        # build()
    else:
        app()
